...
//            imageIcon,
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: <Widget>[
                  BotResponse(message: item.title,),
                  item.isUserResponded?UserResponse(response: item.userResponse,):_getButton(item.buttons,userResponse)
                ],
              ),
            ),
          ],
    ...
    child: Row(
      ...
      children: <Widget>[
        ...
        Padding(
          ...
          child: Image.network(imageUrl,height: 200,width: 250,fit: BoxFit.fill,),
        ),
      ],
    ),
  );
...
Widget _getButton(List<InteractionsResponsesButton> buttons,UserResponseClick userResponse){
  return Center(
    child: Column(
...
Widget _getCardButtons(List<InteractionsResponsesButton> buttons,UserResponseClick userResponse){
  return Center(
    child: Column(
        children: List.generate(buttons.length,(index){
          return ButtonTheme(
...
class UserResponse extends StatelessWidget{
  ...
  Widget build(BuildContext context) {
    return Align(
      ...
      child: response.isLoading!=null&&!response.isLoading?Column(
        ...
        children: <Widget>[
          Container(
            ...
            child: Text(response.title,style: TextStyle(color: Colors.white),),
          ),
          timeWidget()
        ],
      ):Padding(
            ...
            child: Card(
              child: Column(
                ...
                children: <Widget>[
                  ...
                  Padding(
                    padding: const EdgeInsets.all(8.0),
                    child: response.isUserResponded?UserResponse(response: response.userResponse,):_getCardButtons(response.buttons,userResponse)
                  )
                ],
              ),
            ),
...
Widget timeWidget(){
  return Container(
      ...
      child: Text(formattedDate(DateTime.now()),style: TextStyle(fontSize: 12),));
}


Widget imageIcon = Image.network("https://cdn.pixabay.com/photo/2017/10/24/00/39/bot-icon-2883144_960_720.png",width: 30,height: 30,);

String formattedDate(DateTime now) {return DateFormat('hh:MM a').format(now);}